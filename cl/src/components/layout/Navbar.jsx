...
function FocusOverlay({ onClose }) {
      ...
      intervalRef.current = setInterval(() => {
        setSeconds(s => {
          if (s <= 1) {
            ...
            setRunning(false);
            if (session === "work") { setSession("break"); setSeconds(breakSecs); setRounds(r => r + 1); }
            else                    { setSession("work");  setSeconds(workSecs);  setBreaksCompleted(b => b + 1); }
            return 0;
          }
          return s - 1;
        });
  ...
  const reset = () => { setRunning(false); setSession("work"); setSeconds(PRESETS[preset].work * 60); };

  const changePreset = (i) => { setPreset(i); setRunning(false); setSession("work"); setSeconds(PRESETS[i].work * 60); };
      ...
      <div style={FO.panel} onClick={e => e.stopPropagation()}>
        ...
        <div style={FO.presets}>
          {PRESETS.map((p, i) => (
            ...
            >
              {p.label}
              <span style={{ fontSize:11, opacity:0.7, marginLeft:4 }}>{p.work}m/{p.break}m</span>
            </button>
          ))}
        </div>
        ...
        <div style={FO.timerWrap}>
          <svg width={200} height={200} style={{ transform:"rotate(-90deg)" }}>
            <circle cx={100} cy={100} r={R} fill="none" stroke="rgba(124,58,237,0.15)" strokeWidth={8}/>
            <circle cx={100} cy={100} r={R} fill="none"
              stroke={session === "work" ? "#a855f7" : "#10b981"}
              strokeWidth={8} strokeDasharray={C} strokeDashoffset={arc}
        ...
        <div style={FO.controls}>
          ...
          <button
            style={{ ...FO.playBtn, background: running ? "linear-gradient(135deg,#ef4444,#dc2626)" : "linear-gradient(135deg,#7c3aed,#a855f7)" }}
          ...
          <button style={FO.resetBtn} onClick={() => {
            setRunning(false);
            if (session === "work") { setRounds(r => r + 1); setSession("break"); setSeconds(breakSecs); }
            else                    { setBreaksCompleted(b => b + 1); setSession("work"); setSeconds(workSecs); }
          }}>⏭</button>
        </div>
        ...
        <div style={FO.tip}><span style={{ fontSize:13 }}>{tip}</span></div>
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          ...NAV.bar,
          // On mobile shift right to avoid hamburger button
      ...
      >
        ...
        <div style={{ position:"relative" }} ref={modeRef}>
          ...
          >
            ...
            {!isMobile && <span style={NAV.modeLabel}>{activeMode.label}</span>}
          ...
          {showModes && (
            <div style={{
              ...NAV.modeDropdown,
              // On mobile make it wider and position more carefully
